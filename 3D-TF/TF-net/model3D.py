...
class LES(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, dropout_rate, time_range):
        ...
        self.temporal_filter = nn.Conv3d(time_range, 1, kernel_size = 1, padding = 0, bias = False)
        ...
        self.output_layer = nn.Conv3d(32 + input_channels, output_channels, kernel_size=kernel_size,
                                      padding=(kernel_size - 1) // 2)
        
    def forward(self, xx):
        xx_len = xx.shape[1]
        # u = u_mean + u_tilde + u_prime
        u_tilde = self.spatial_filter(xx.reshape(xx.shape[0]*xx.shape[1], 1, 128, 128, 128)).reshape(xx.shape[0], xx.shape[1], 128, 128, 128)
        # u_prime
        u_prime = (xx - u_tilde)[:,(xx_len - self.input_channels):]
        # u_mean
        u_tilde2 = u_tilde.reshape(u_tilde.shape[0], u_tilde.shape[1]//3, 3, 128, 128, 128)
        u_mean = []
        for i in range(xx_len//3 - self.input_channels//3, xx_len//3):
            cur_mean = torch.cat([self.temporal_filter(u_tilde2[:,i-self.time_range+1:i+1,0,:,:,:]).unsqueeze(2), 
                                  self.temporal_filter(u_tilde2[:,i-self.time_range+1:i+1,1,:,:,:]).unsqueeze(2),
                                  self.temporal_filter(u_tilde2[:,i-self.time_range+1:i+1,2,:,:,:]).unsqueeze(2)], dim = 2)
            u_mean.append(cur_mean)
        u_mean = torch.cat(u_mean, dim = 1)
        u_mean = u_mean.reshape(u_mean.shape[0], -1, 128, 128, 128)
        # u_tilde
        u_tilde = u_tilde[:,(self.time_range-1)*3:] - u_mean
