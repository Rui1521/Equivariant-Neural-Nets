import os #Magnitude Equivariant Neural Nets
import time
import torch
import numpy as np
...
from model import U_net#ResNet, 
...
for i in range(10):
    ...
    if valid_mse[-1] < min_mse:
        min_mse = valid_mse[-1] 
        best_model = model
        torch.save(model, "Unet-Mag"+idx+".pth")
    end = time.time()
...
best_model = torch.load("Unet-Mag"+idx+".pth")
loss_fun = torch.nn.MSELoss()
...
test_direc = "/global/cscratch1/sd/rwang2/TF-net/Data/mag_64/sample_"
test_set = Dataset(test_indices, input_length, 40, 10, test_direc, True)
test_loader = data.DataLoader(test_set, batch_size = batch_size, shuffle = False, num_workers = 8)
valid_mse, preds, trues, loss_curve = test_epoch(test_loader, best_model, loss_fun)
